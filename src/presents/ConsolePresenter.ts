  ...
  while (option != "5") {
    ...
    switch (choice) {
      ...
      case "2":
        ...
        if (barcodeInput !== null) {
          ...
          if (productToBuy) {
            ...
            console.log(
              "Carrinho atualizado:",
              productsInCart.map((data) => ProductViewModel.viewProduct(data))
            );
          } else {
      ...
      case "4":
        ...
        console.log(
          "Produtos:",
          productsInShoppingCart.map((data) =>
            ProductViewModel.viewProduct(data)
          )
        );
        const barcodeKeyboardValue = keyboard(
        ...
        if (barcodeKeyboardValue !== null) {
          ...
          const productToRemove = productsInShoppingCart.find(
            (product) => product.barcode === barcodeToRemove
          );
          if (productToRemove) {
            cli.removeProductFromShoppingCart(barcodeToRemove);
            console.log(
              `Produto '${productToRemove.name}' removido do carrinho.`
            );
